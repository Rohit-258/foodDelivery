...
const CreateContainer = () => {
  ...
  const uploadImage =(e)=>{
    ...
    uploadTask.on('state_changed',(snapshot)=>{
      ...
      setMsg("Uploading Failed! Try Again.")
      setTimeout(() => {
        setFields(false);
        setIsLoading(false);
      }, 4000);
  ...
  const saveDetails = ()=>{
    setIsLoading(true);
    try {
      if((!imageAsset || !title || !calories ||!category || !price)){
        setFields(true);
        setAlertStatus("danger");
        setMsg("Required fields have to be filled.");
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
        }, 4000);
    ...
    <div className=" w-full min-h-screen flex items-center justify-center">
      ...
      >
        ...
        <div className='flex items-center w-full'>
          <button className='ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 px-12 py-2 rounded-lg text-lg text-white font-semibold'
          onClick={saveDetails}
          >
            Save
          </button>
